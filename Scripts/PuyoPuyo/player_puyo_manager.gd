...
@export var sfx_manager : PuyoSoundManager
...
func _physics_process(delta: float) -> void:
	if player_input_flag:
		if Input.is_action_just_pressed("puyo_left"):
			...
			if grid_manager.check_next_move(next_grid_positions):
...
func create_player_puyo():
	if !(await grid_manager.loss_check()):
		...
		var end_flag = false
		if !(grid_manager.check_next_move(player_grid_positions)):
			for i in range(0, grid_width - 1):
				position_check = [Vector2i(i, 0), Vector2i(i + 1, 0)]
				if (grid_manager.check_next_move(position_check)):
					end_flag = true
					break
			if end_flag == false:
				life_loss.emit()
...
func player_down_tick():
	...
	if grid_manager.check_next_move(next_moves):
		...
		for i in range(0,2):
			...
			if (!(grid_manager.check_next_move([next_moves[i]]))) or player_rotation == 1 or player_rotation == 3:
...
func player_rotate():
	...
	if rotation_check == 0: #now pointing right
		if !(grid_manager.check_next_move(move_check)): #if the new rotated vector would be invalid
			for i in range(0, move_check.size()):
				move_check[i].x -= 1
			if (grid_manager.check_next_move(move_check)): #if moving both puyos one to the left would be valid
		...
		if !(grid_manager.check_next_move(move_check)):
			for i in range(0, move_check.size()):
				move_check[i].y -= 1
			if (grid_manager.check_next_move(move_check)):
				player_rotation = rotation_check
